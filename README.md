...
| `TRUSTED_PROXY_CIDRS` | Comma-separated address ranges of the reverse proxies in front of the BFF (e.g. `10.244.0.0/16`); a bare address is read as a `/32`. Empty (default) ignores `X-Forwarded-For` and throttles on the direct peer — behind an ingress that is the controller's address for **every** user, so one person fumbling a password throttles everyone, break-glass included. The client IP is resolved by walking from the TCP peer and popping `X-Forwarded-For` entries off the **right** only while each hop falls inside these ranges, so a caller who prepends to the header gets only what the proxy actually observed, and one who skips the proxy entirely never has their header read. Same shape as the gateway's `security.trusted_proxy_cidrs`. `TRUST_FORWARDED_FOR` and `TRUSTED_PROXY_HOPS` are **refused at startup** — both said "trust this header" without saying whose hops are yours |
